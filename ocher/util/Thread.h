...
inline thread_id Thread::currentThreadId()
{
    return (Thread *)*taskdata();
}
inline void Thread::yield()
{
    assert(currentThreadId() == m_thread);
    taskyield();
}
inline void Thread::yieldCurrent()
{
    taskyield();
}
#elif defined(__HAIKU__)
...
{
    return m_state == STARTING || m_state == RUNNING;
}
inline bool Thread::isInterrupted() const
{
    return m_flags & INTERRUPTED;
}
